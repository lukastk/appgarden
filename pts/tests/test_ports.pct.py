...
# ## merge_allocations
#
# Reconciles already-deployed app ports (e.g. read from a garden's garden.json)
# into the shared host-level registry. Used by `server init` to migrate existing
# gardens into the one box-global ports.json.
...
# %% [markdown]
# ## Remote-aware functions use the shared host-level registry
#
# `allocate_port` / `release_port` / `get_app_port` take no `ctx`: ports are a
# box-global resource, so every garden on a host allocates from one registry.
# This is what stops two gardens on one host handing out the same host port.
